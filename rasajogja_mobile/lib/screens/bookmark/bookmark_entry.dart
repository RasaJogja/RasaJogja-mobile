...
class _BookmarkPageState extends State<BookmarkPage> {
  Future<List<BookmarkedProduct>> fetchBookmarks() async {
    final url = Uri.parse('http://127.0.0.1:8000/bookmark/bookmarked-products_flutter/');
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return List<BookmarkedProduct>.from(
          responseData['bookmarked_products'].map((x) => BookmarkedProduct.fromJson(x)),
        );
      } else {
  ...
  Future<void> deleteBookmark(int productId) async {
    final url = Uri.parse('http://127.0.0.1:8000/bookmark/remove_flutter/$productId/');
    try {
                  ...
                  child: ListTile(
                    ...
                    trailing: IconButton(
                      ...
                      onPressed: () async {
                        ...
                        if (confirm == true) {
                          await deleteBookmark(product.id); // Panggil metode delete
                        }
                      },
                    ),
                  ),
...
class BookmarkedProduct {
  ...
  factory BookmarkedProduct.fromJson(Map<String, dynamic> json) => BookmarkedProduct(
        id: json["id"],
        nama: json["nama"],
